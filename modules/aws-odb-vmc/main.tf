locals {
  ...
  db_servers = var.db_servers == null ? [for db_server in var.aws_exainfra_dbservers.db_servers : db_server.id] : var.db_servers
...
data "aws_odb_db_nodes" "this" {
  cloud_vm_cluster_id = aws_odb_cloud_vm_cluster.this.id
}